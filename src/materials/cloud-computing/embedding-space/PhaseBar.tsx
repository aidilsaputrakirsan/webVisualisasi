...
import { PHASES, type Phase } from './embeddings'

const ORDER: Phase[] = ['embed', 'cluster', 'search']

/**
 * Breadcrumb of the 3 phases (Embed -> Cluster -> Search) for the single
 * continuous flow. Active phase is indigo, finished phases green, upcoming grey.
 */
        ...
        return (
          <div key={p} className="flex items-center" style={{ gap: 13 }}>
            ...
            >
              ...
              <span className="font-semibold" style={{ fontSize: 23, color: lit ? st.text : '#9FA3BC', whiteSpace: 'nowrap' }}>
                {PHASES[p].label}
              </span>
            </motion.div>
            {i < ORDER.length - 1 && (
              <motion.span animate={{ color: i < activeIdx ? NODE.done.border : '#C7CADD' }} style={{ fontSize: 26 }}>
                →
              </motion.span>
            )}
          </div>
