...
import AdvSortView from './AdvSortView'
...
const BADGES = [
  { label: 'TIME', value: 'O(n log n)', color: '#3b82f6' },
  { label: 'D&C', value: 'divide & conquer', color: '#a855f7' },
]

export default function AdvancedSortingMaterial() {
  ...
  useEffect(() => {
    ...
    const pitch = step.compareIds.length ? step.cells.find((c) => c.id === step.compareIds[0])?.value ?? 30 : 30
    switch (step.sound) {
      case 'compare':
        playCompare(pitch)
        break
    ...
    <>
      <MaterialStage>
        <div className="flex h-full w-full flex-col items-center justify-center" style={{ paddingTop: 96, paddingBottom: 130, gap: 40 }}>
          <TitleBlock title="SORTING LANJUTAN" subtitle={def.desc} badges={BADGES} />

          <AdvSortView step={step} />
          ...
          <CodeBlock filename={def.filename} source={def.code} activeLine={step.line} fontSize={mode === 'heap' ? 18 : 20} />
